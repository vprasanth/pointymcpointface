...
- `/points` shows a top-10 leaderboard, `/points leaderboard week|month` scopes it to recent activity, `/points history @user` shows recent awards, `/points stats` shows top givers/receivers, `/points @user` shows a single user's total, and `/points give @user reason` awards points.
